...
from ..core.config import GRID_CONFIG_DIR, OUTPUT_DIR
from ..core.db import get_db
from ..services.astar import run_astar_process, find_nearest_available_path
...
async def save_grid_config(payload: dict):
    ...
    for cell in parking_spaces:
        if not is_valid_cell(cell, rows, cols):
            raise HTTPException(status_code=400, detail="one or more parking space cells are invalid")

    get_grid_file_path(camera_id).write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def create_grid_source_from_video(
    ...
    await asyncio.get_event_loop().run_in_executor(None, input_path.write_bytes, file_bytes)
...
async def run_pathfinder(camera_id: str, payload: dict):
    ...
    rows, cols, grid = grid_data["rows"], grid_data["cols"], grid_data["grid"]
    ...
    return await asyncio.get_event_loop().run_in_executor(
        None, lambda: run_astar_process(camera_id, rows, cols, start, goal, grid)
    )


@router.post("/run-nearest/{camera_id}")
async def run_nearest_available_path(
    camera_id: str, payload: dict, db: AsyncIOMotorDatabase = Depends(get_db)
):
    file_path = get_grid_file_path(camera_id)
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="grid config not found")
    ...
    return await asyncio.get_event_loop().run_in_executor(
        None,
        lambda: find_nearest_available_path(camera_id, rows, cols, start, grid, parking_spaces, spots_doc),
    )
