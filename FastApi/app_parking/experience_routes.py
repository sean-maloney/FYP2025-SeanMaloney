...
from .astar_service import find_nearest_available_path
from .config import OUTPUT_DIR, UPLOAD_DIR, GRID_CONFIG_DIR
...
async def run_experience(
    ...
    if not spots_doc:
        raise HTTPException(
            status_code=400,
            detail="no published spot config for this camera_id"
        )
    ...
    if not grid_file.exists():
        raise HTTPException(
            status_code=400,
            detail="no saved grid config for this camera_id"
        )
    ...
    if not start or len(start) != 2:
        raise HTTPException(
            status_code=400,
            detail="grid config is missing a valid start cell"
        )
